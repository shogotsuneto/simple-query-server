...
func TestJWKSClient_KidBasedCacheOptimization(t *testing.T) {
	requestCount := 0
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		requestCount++
		mockJWKS := `{
			"keys": [
				...
				},
				{
					...
					"kid": "test-key-2",
					"use": "sig",
					"n": "1vx7agoebGcQSuuPiLJXZptN9nndrQmbXEps2aiAFbWhM78LhWx4cbbfAAtVT86zwu1RK7aPFFxuhDR1L6tSoc_BJECPebWKRXjBZCiFV4n3oknjhMstn64tZ_2W-5JsGY4Hc5n9yBXArwl93lqt7_RN5w6Cf0h4QyQ5v-65YGjQR0_FDW2QvzqY368QQMicAtaSqzs8KJZgnYb9c7d0zgdAZHzu6qMQvRL5hajrn1n91CbOpbISzIWzYr_W6UU9dwuW6TU0DjW0nQcaOLGOjQhGnOGKZ9CW7PDNE2J",
					"e": "AQAB"
				}
			]
		}`
	...
	// First request should fetch from server
	key1, err := client.GetPublicKey("test-key-1")
	if err != nil {
		t.Fatalf("Expected no error, got %v", err)
	}
	if key1 == nil {
		t.Fatal("Expected key1, got nil")
	}
	if requestCount != 1 {
		t.Fatalf("Expected 1 request, got %d", requestCount)
	}

	// Second request for same key should use cache (optimization working)
	key1Again, err := client.GetPublicKey("test-key-1")
	if err != nil {
		t.Fatalf("Expected no error, got %v", err)
	}
	if key1Again == nil {
		t.Fatal("Expected key1Again, got nil")
	}
	if requestCount != 1 {
		t.Fatalf("Expected 1 request (same key cached), got %d", requestCount)
	}

	// Request for different key should use same cache
	key2, err := client.GetPublicKey("test-key-2")
	if err != nil {
		t.Fatalf("Expected no error, got %v", err)
	}
	if key2 == nil {
		t.Fatal("Expected key2, got nil")
	}
	if requestCount != 1 {
		t.Fatalf("Expected 1 request (different key same cache), got %d", requestCount)
	}

	// Wait for cache to expire
	time.Sleep(150 * time.Millisecond)

	// Request after cache expiry should fetch again
	_, err = client.GetPublicKey("test-key-1")
	if err != nil {
		t.Fatalf("Expected no error, got %v", err)
	}
	...
	// Request for non-existent key should still use the fresh cache
	_, err = client.GetPublicKey("non-existent")
	if err == nil {
		t.Fatal("Expected error for non-existent key")
	}
	if requestCount != 2 {
		t.Fatalf("Expected 2 requests (non-existent key should not fetch again), got %d", requestCount)
	}
}
